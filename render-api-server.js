...
app.post('/api/session', async (req, res) => {
    const { homeworkDueDate, schoolName, username, password } = req.body;
    
    if (!homeworkDueDate || !schoolName || !username || !password) {
        return res.status(400).json({ 
            error: 'Missing required fields: homeworkDueDate, schoolName, username, password' 
        });
    }
    ...
    jobQueue.push({
        id: jobId,
        type: 'session', // Mark this as a session job
        homeworkDueDate,
        schoolName,
        username,
        password,
    ...
    const maxWaitTime = 300000; // 5 minutes for solving homework
    const checkInterval = 1000;
    let waited = 0;
    ...
    while (waited < maxWaitTime) {
        if (jobResults[jobId]) {
            ...
            return res.json({
                success: result.success,
                message: result.success ? 'Homework completed!' : 'Session failed',
                error: result.error || null,
